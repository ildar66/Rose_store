...
/////////////////////////////////////////////////////////Блок удаления элементов:
   void delete(Element e)   //удаление элемента "e"
	 {
	   ...
	   if(e instanceof Figure)        //удаление фигуры"e"
		{
		 while(e.elements.size() != 0)
		   delete((Side)e.elements.lastElement());  //удаление сторон фигуры"e"
		} 
	   if(e instanceof Side)          //удаление стороны"e"
		{
		 copy = (Side)e;    //сохранение "e" в "copy"
		 while(e.elements.size() != 0)
		   delete((Element)e.elements.lastElement());  //удаление элементов стороны"e"
   ...
   void deletelast3D(Element e)  //удаление последней 3Dточки элемента "e"
	 {
	   if(e.field.size() != 0)
		{
   ...
   private void trim3D(int t, Element elem)   //упорядочение элемента "elem" после удаления "t"-ой 3Dточки
	 {
...
///////////////////////////////////////////////////////Блок вращения элементов:
   synchronized void rotate(String axis, Element e, float rotate) //вращение Element-a "e" вокруг оси координат"axis"
	 {                                                      //на угол "rotate"
	   matrix.identity();
   ...
   synchronized void rotate(Element elem, int a, int b, float rotate)  //new : вращение Element-a "e" вокруг оси(a,b)
	 {                                                                 //на угол=rotate
	  if ((a!=b) && (Math.max(a,b)< points3D.size())) 
	   {
		float animX, animY, animZ;
		matrix.identity();
		Point3D pA = (Point3D)points3D.elementAt(a);
		Point3D pB = (Point3D)points3D.elementAt(b);
		Vector3D axis = new Vector3D(pB.x-pA.x, pB.y-pA.y, pB.z-pA.z); // вектор оси вращения
		animX = axis.x/axis.mag() * rotate; 
		animY = axis.y/axis.mag() * rotate; 
		animZ = axis.z/axis.mag() * rotate; 
...
///////////////////////////////////////////////////////общий Блок:
  private synchronized void transform(Matrix3D m, Vector v) //метод, вызываемый после изменения СК
	 {
